function UERemoteGeneralAPI() {
  ...
  var g_temp = { handle: null, trashold: 50 };
  function trace(str) {
    console.log(str);
  }
  ...
  function validateInited() {
    ...
    if (isInited == false) throw new Error("The remote parent not inited");
  }
  ...
  function getNumCurrent() {
    ...
    if (objCurrent.length == 0) return -1;
  ...
  function getItem(index) {
    ...
    if (index >= objItems.length) index = objItems.length - 1;
  ...
  function unselectItems() {
    ...
    if (g_vars.enableDebug) trace("Items Unselected: " + g_vars.parent_id);
  }
  function changeItem(mixed) {
    var numItem = getItemNum(mixed);
    var objItem = getItem(numItem);
    if (objItem == null) {
      if (g_vars.enableDebug) trace("General API: changeItem - item not found");
      return false;
    }
    ...
    if (numCurrent === numItem) {
      if (g_vars.enableDebug)
        trace(
          "General API: changeItem - num current == numitem - skip (" +
            numItem +
            " )"
        );
      return false;
    ...
    if (objInner.length == 0) {
      trace(objItem);
      throw new Error(
        "Can't find inner by selector:" + g_vars.selector_item_trigger
      );
    }
  ...
  this.doAction = function (action, arg1, arg2) {
    ...
    if (g_isTypeEvents == true) {
      ...
      if (!funcRunAction) throw new Error("Missing option: 'func_doAction' ");
      var response = g_options.func_doAction(action, arg1, arg2);
      return response;
    }
    if (g_vars.enableDebug) {
      trace(
        "Action (General): " + action + " arg1: " + arg1 + " arg2: " + arg2
      );
    }
    switch (action) {
      case "get_num_current":
        var current = getNumCurrent();
        if (g_vars.enableDebug) trace("response: " + current);
        return current;
        break;
      case "get_total_items":
        var total = getNumTotal();
        if (g_vars.enableDebug) trace("response: " + total);
        return total;
        break;
      case "change_item":
        changeItem(arg1);
  ...
  function initEvents_setActive() {
    if (g_vars.enableDebug == true) trace("start initEvents_setActive");
    var objItems = getObjItems();
    if (objItems.length == 0) {
      if (g_vars.enableDebug == true) trace("no items, exit");
      return false;
    }
    var objFirstItem = getItem(0);
    if (objFirstItem == null) {
      if (g_vars.enableDebug == true) trace("empty first item - exit");
      return false;
    }
    if (g_vars.active_code_first_unselected !== true)
      ...
      trace(
        "skip activating first item: g_vars.active_code_first_unselected=true"
      );
  ...
  function initEvents() {
    ...
    if (g_vars.listen_class_change == true) initEvents_listenClassChange();
  ...
  function initByClasses() {
    try {
      ...
      g_vars.active_code_first_unselected = getVal(
        g_options,
        "active_code_first_unselected"
      );
      if (g_vars.add_set_active_code === true)
  ...
  this.init = function (objParent, options, isEditor) {
    ...
    if (connectType == "events") g_isTypeEvents = true;
    g_options = options;
    var enableDebug = getVal(options, "trace_debug");
    if (enableDebug == true) g_vars.enableDebug = true;
    if (g_vars.enableDebug == true) {
      trace("init general api");
      trace(objParent);
      trace(options);
    }
    if (isEditor == true) g_vars.trigger_event = "ucclick";
    if (g_isTypeEvents == false) initByClasses();
    g_objParent.data("remote_inited", true);
    if (g_isTypeEvents == false) {
      if (g_vars.listen_class_change == true) setTimeout(initEvents, 1000);
      else initEvents();
...
function UERemoteCarouselAPI() {
  ...
  function resetAutoplay() {
    if (g_owl.settings.autoplay == false) return false;
  ...
  this.doAction = function (action, arg1, arg2) {
    ...
    switch (action) {
      ...
      case "pause":
        ...
        break;
      case "is_playing":
        if (g_owl.settings.autoplay == true) return true;
        else return false;
        break;
      case "get_total_items":
      ...
      case "get_modified_progress_time":
        var progressTime =
          (g_owl.settings.autoplayTimeout - g_owl.settings.smartSpeed) / 1000;
        return progressTime;
        break;
      case "get_num_current":
      ...
      case "change_item":
        ...
        var currentItem = g_owl.relative(g_owl.current());
        var slideNum = arg1;
        if (slideNum == currentItem) return false;
        if (slideNum >= total) slideNum = total - 1;
        else if (slideNum < 0) slideNum = 0;
...
function UESyncObject() {
  ...
  function validate(objAPI) {
    if (g_objApis.length == 0) return false;
    ...
    for (var elID in g_objApis) {
      ...
      if (numItemsExisting !== numItems) {
        ...
        throw new Error(
          "Sync failed, number of items should be the same. Now it's " +
            numItems +
            " and " +
            numItemsExisting
        );
  ...
  this.setOptions = function (groupName, isEditor) {
    if (isEditor === true) g_vars.is_editor = true;
    if (!g_groupName) g_groupName = groupName;
  };
  ...
  function mapAPIs(func, objElement) {
    if (typeof ucRemoteDebugEnabled != "undefined") g_vars.show_debug = true;
    var elementID = null;
    if (objElement) {
      var elementID = objElement.attr("id");
      if (g_objApis.length == 1) return null;
    }
    for (var elID in g_objApis) {
      var api = g_objApis[elID];
      if (elementID && elID == elementID) continue;
      func(api);
    }
  }
  function onItemChange(objAPI) {
    ...
    mapAPIs(function (api) {
      if (numCurrent < 0) api.doAction("unselect_items");
      else api.doAction("change_item", numCurrent);
    }, objElement);
  }
  ...
  this.getDebugText = function (objElement) {
    ...
    mapAPIs(function (api) {
      ...
      if (textWidgets) textWidgets += ", ";
      textWidgets +=
        "<a href='#" +
        widgetID +
        "' style='color:green;text-decoration:underline'><b>" +
        widgetName +
        "</b></a>";
    }, objElement);
    if (textWidgets) text += "sync with: " + textWidgets;
    return text;
  };
  function removeDeletedAPIs() {
    ...
    mapAPIs(function (api) {
      ...
      if (objElement.length == 0) return false;
  ...
  this.addAPI = function (objAPI) {
    ...
    if (g_objApis.hasOwnProperty(id)) return false;
    g_objApis[id] = objAPI;
    if (g_vars.is_editor == true) removeDeletedAPIs();
    validate(objAPI);
    var objElement = objAPI.getElement();
    trigger("update_debug", g_groupName);
  ...
  this.getElements = function (exceptID) {
    if (!g_objApis) return null;
    var arrElements = [];
    for (var elID in g_objApis) {
      if (exceptID && exceptID == elID) continue;
      var objApi = g_objApis[elID];
      var objElement = objApi.getElement();
      arrElements.push(objElement);
    }
    if (arrElements.length == 0) return null;
    return arrElements;
  };
}
function UERemoteWidgets() {
  ...
  var g_api,
    g_objSync,
    g_remoteConnection = window.ueRemoteConnection;
  var t = this;
  var g_vars = {
    is_inited: false,
    funcOnInit: null,
  ...
  function initWidget(widgetID) {
    ...
    if (forceID) g_parentID = forceID;
    else g_parentID = g_objWidget.data("parentid");
    if (!g_parentID) throw new Error("Parent controlled ID not set");
  }
  function getParentType() {
    if (!g_objParent || g_objParent.length == 0)
      throw new Error("getParentType: no parent found");
    if (g_objParent.hasClass("owl-carousel")) return g_types.CAROUSEL;
  ...
  function filterParentsBySection(objParents, objElement) {
    ...
    if (objSection.length == 0) return objParents;
    ...
    if (objFiltered.length) return objFiltered;
    return objParents;
  }
  function detectClosestParent(objParents) {
    ...
    if (objTemplateHolder.length) objContainer = objTemplateHolder;
    if (!objParents)
    ...
    if (numParents == 0) return null;
    if (numParents == 1) return objParents;
    if (g_vars.trace_debug) trace("filter auto");
    var objParentsFiltered = objParents.filter(
      "[data-remoteid='" + g_parentID + "']"
    );
    if (objParentsFiltered.length == 1) return objParentsFiltered;
    if (g_vars.trace_debug) {
    ...
    if (objParents.length == 1) return objParents;
    if (g_vars.trace_debug) trace("get closest by parent");
    var objClosest = getClosestByOffset(objParents, g_objWidget);
    if (objClosest) return objClosest;
    if (g_vars.trace_debug) trace("get first");
  ...
  function setParentObject() {
    ...
    if (g_vars.trace_debug)
      trace(
        "start set parent for: " + widgetID + ", parent name: " + g_parentID
      );
    if (objForceParent) {
    ...
    if (!g_parentID) throw new Error("Parent controller ID not found");
    if (!g_objParent || g_objParent.length == 0) {
      if (g_parentID == "auto") {
        if (g_vars.trace_debug) {
        ...
        if (!g_objParent) throw new Error("Can't detect remote parent");
      } else {
        var objParents = jQuery(
          ".uc-remote-parent[data-remoteid='" + g_parentID + "']"
        ).not(g_objWidget);
        if (g_vars.trace_debug) trace("Detect from group: " + g_parentID);
        ...
        if (!g_objParent || g_objParent.length == 0) {
          ...
          if (!isAnotherTry) {
            ...
            if (g_vars.trace_debug) trace("Set another try for parent detect");
            return false;
          }
          throw new Error(
            "Parent widget with remote name: '" + g_parentID + "' not found"
          );
        }
      }
    }
  ...
  function initAPI() {
    ...
    if (optionsFromData) g_vars.options_api = optionsFromData;
    var optionsAPI = {};
    ...
    if (g_vars.trace_debug == true) {
      if (optionsAPI) trace(optionsAPI);
  ...
  function initParent() {
    ...
    if (isParentSet == false) return false;
    var isInited = initAPI();
    return isInited;
  }
  function initGlobal(widgetID, func) {
    if (!g_objWidget) initWidget(widgetID);
    ...
    if (g_vars.trace_debug) {
      if (!widgetID) widgetID = g_objWidget.attr("id");
  ...
  this.setAction = function (action, objElement, allowMultiple) {
    ...
    if (g_vars.is_inited == false) throw new Error("Widget not inited");
    ...
    if (!g_api) throw new Error("API not inited!");
    ...
    objElement.on("click", function () {
      ...
      if (objElement.hasClass("uc-disabled")) return true;
      t.doAction(action);
    });
  };
  ...
  function hideErrorOnWidget() {
    if (!g_objWidget || g_objWidget.length == 0) return false;
    var objParent = getWidgetContainer(g_objWidget);
    var objError = objParent.find(".uc-remote-error");
    if (objError.length == 0) return false;
    if (objError.is(":visible") == false) return false;
    objError.hide();
  ...
  function getWidgetContainer(objWidget) {
    ...
    if (objParent.length == 0) objParent = objWidget.parent();
    return objParent;
  }
  function addTextDiv(objWidget, type) {
    ...
    if (isDebug == true) className = "uc-remote-debug";
    ...
    var objParentsBG = objParent.parents(
  ...
  function displayTextOnWidget(objWidget, message, type) {
    ...
    if (isDebug == false) {
      objWidget.css({ border: "2px solid red", position: "relative" });
    }
    objText.show();
  ...
  function isDebugActive(objWidget) {
    if (!objWidget) objWidget = g_objWidget;
  ...
  function removeDebugVisual(objWidget) {
    ...
    g_objWidget.css({ "border-style": "none" });
  }
  ...
  function checkWidgetDebugWork(objWidget, objParent) {
    if (!objWidget) objWidget = g_objWidget;
    if (!objParent) objParent = g_objParent;
    var isDebug = isParentDebugActive(objParent);
    var isActive = isDebugActive(objWidget);
    if (isDebug == false) {
      if (isActive == true) removeDebugVisual(objWidget);
      return false;
    }
    if (isActive == false) {
  ...
  function checkDebugConnectionText() {
    if (!g_objWidget) return false;
    if (g_vars.show_connection_debug == false) return false;
    if (!g_objParent || g_objParent.length == 0)
  ...
  function checkWidgetDebug() {
    ...
    if (noDebugCheck === true) return false;
  ...
  this.doAction = function (action, arg1, arg2) {
    ...
    if (!g_api) {
      if (g_vars.trace_debug) trace("empty g_api, exit");
      return false;
    }
    switch (action) {
      case "prev":
  ...
  function updateSyncDebug(event, syncID) {
    if (g_vars.debug_show_ids == true) return false;
    try {
      if (syncID != g_vars.syncid) {
        var name = g_objParent.data("widgetname");
        throw new Error(
          "Wrong sync group mishmash " + g_vars.syncid + " and " + syncID
        );
      }
  ...
  function isInsideEditor() {
    if (g_vars.is_editor !== null) return g_vars.is_editor;
    ...
    if (
      ...
      typeof window.parent.elementor !== "undefined"
    ) {
      g_vars.is_editor = true;
      return true;
    }
  ...
  function checkWidgetInsideEditor() {
    try {
      ...
      if (g_vars.is_inited == true) {
        ...
        if (g_vars.is_inited == true) {
          g_vars.funcOnInit();
        } else if (g_vars.is_inited == false) {
  ...
  this.onWidgetInit = function (widgetID, func, options) {
    try {
      if (g_vars.is_inited == true) return false;
      if (g_vars.trace_debug == true) {
      ...
      if (!g_vars.funcOnInit) {
        if (typeof func != "function")
          throw new Error(
            "onWidgetInit error: the second parameter should be a function"
          );
        g_vars.funcOnInit = func;
      }
      if (options && g_vars.init_options == null) g_vars.init_options = options;
      ...
      if (g_vars.trace_debug == true) trace("start debug - show connect");
      ...
      displayErrorMessage(message);
      var isEditorMode = isInsideEditor();
      if (isEditorMode == true) setInterval(checkWidgetInsideEditor, 700);
      return false;
    }
  };
  function startParentSync() {
    ...
    if (isInited == false) {
      ...
      if (parentType == g_types.CAROUSEL) {
        message +=
          ", please check that the owl carousel js file loading from unlimited elements plugin.";
      } else {
  ...
  this.onParentInit = function (objParent, optionsAPI) {
    try {
      g_objParent = objParent;
      if (!g_objParent) return false;
      if (g_objParent.length == 0) return false;
      g_vars.is_parent_mode = true;
      var optionsFromData = g_objParent.data("uc-remote-options");
      if (optionsFromData) optionsAPI = optionsFromData;
      if (optionsAPI) g_vars.options_api = optionsAPI;
      var isDebug = g_objParent.data("debug");
      if (typeof ucRemoteDebugEnabled != "undefined") isDebug = true;
      g_vars.trace_debug = isDebug;
      if (isDebug === true) addParentDebug(objParent);
      var isSync = objParent.data("sync");
      if (isSync == true) startParentSync();
      if (g_vars.debug_show_ids == true) {
...
function UERemoteConnection() {
  ...
  this.getVal = function (obj, name, defaultValue) {
    ...
    var val = "";
    if (!obj || typeof obj != "object") val = defaultValue;
  ...
  this.getSyncedElements = function (objElement) {
    ...
    if (objElement.length == 0) return null;
    if (objElement.hasClass("uc-remote-parent") == false) return null;
    var isSync = objElement.data("sync");
    if (isSync !== true) return null;
...
jQuery(document).on(
  "uc-remote-parent-init",
  function (event, objParent, optionsAPI) {
