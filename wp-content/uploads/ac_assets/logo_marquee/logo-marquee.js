function UCMarquee(marquee) {
  ...
  function wrapSlides() {
    Array.prototype.forEach.call(g_marquee, (item) => {
      item.innerHTML =
        '<div class="uc-logo-wrapper">' + item.innerHTML + "</div>";
    });
  }
  ...
  function setWidth() {
    Array.prototype.forEach.call(g_marquee, (parentItem) => {
      ...
      if (direction == "up" || direction == "down") {
        if (isBreakPoint(767))
          parentItem.style.height =
            (dataHeight + dataMargin) * dataItemsNumberMobile + "px";

        if (isBreakPoint(1024))
          parentItem.style.height =
            (dataHeight + dataMargin) * dataItemsNumberTablet + "px";

        if (isBreakPoint(1920) || viewportWidth > 1920)
          parentItem.style.height =
            (dataHeight + dataMargin) * dataItemsNumberDesktop + "px";
      }
      ...
      Array.prototype.forEach.call(slide, (item) => {
        ...
        if (direction == "right" || direction == "left") {
          if (isBreakPoint(767))
            item.style.width =
              parentWidth / dataItemsNumberMobile - dataMargin + "px";

          if (isBreakPoint(1024))
            item.style.width =
              parentWidth / dataItemsNumberTablet - dataMargin + "px";

          if (isBreakPoint(1920) || viewportWidth > 1920)
            item.style.width =
              parentWidth / dataItemsNumberDesktop - dataMargin + "px";
        }
      });
